...
class TransformVOCDetectionAnnotation(object):
    ...
    def __call__(self, target):
        ...
        for obj in target.iter('object'):
            ...
            bb = obj.find('bndbox')
            bndbox = list(map(tmp_int, [bb.find('xmin').text, bb.find('ymin').text,
        ...
        im_info = list(map(tmp_int,[size.find('height').text, size.find('width').text, 1]))
